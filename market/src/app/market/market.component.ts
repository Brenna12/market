...
export class MarketComponent implements OnInit{
  ...
  ngOnInit(){
    ...
    this.dataService.getAllScenarios().subscribe(res => {
  ...
  onFilterChange(data){
    if(data.type == 'category'){
      if(data.isChecked){
        ...
        this.mainFilter.categories = this.mainFilter.categories.filter(category => { return category.categori_id != data.filter.category })
      }
  ...
  updateProducts(filter){
    ...
    let productsSource = this.productData
    ...
    this.products = productsSource.filter(product => {
      ...
      //     if(!passCategoryFilter){
      //       // passCategoryFilter = this.mainFilter.categories.reduce((found, category) => {
      //         passCategoryFilter = this.mainFilter.categories.reduce((found) => {
      //           console.log(found);
      //           if (product.category == this.mainFilter.categories.categori_id) {
      //             return found;

      //           }


      //           // return found || product.category == category.categori_id
      //       }, false)
      //     }
      //   })
      //   if(!passCategoryFilter){
